class GameBoard {
	...
	/**
	 * Resets the requred functions and variables to restart the game.
	 */
	restart() {
		this.gameOn = true;
		this.counter = 2;
	...
	/**
	 * Increment increases the selection counter for card throw logic. 
	 */
	increment() {
		if (this.gameOn) {
	...
	/**
	 * Decrement decreases the selection counter for card throw logic. 
	 */
	decrement() {
		if (this.gameOn) {
			if (this.counter < 2) {
	...
	/**
	 * Throws away the selected card. Handles counter logic when player card count is low.
	 */
	tossedCard() {
		if (this.gameOn) {
	...
	/**
	 * Updates the card values and graphics.
	 */
	cardUpdate() {
